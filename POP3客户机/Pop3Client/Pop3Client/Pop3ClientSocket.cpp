...
#include "stdafx.h"
#include "Pop3ClientDlg.h" //手动添加包含语句
...
void CPop3ClientSocket::OnReceive(int nErrorCode)
{
	...
	if(nErrorCode==0) 
	{
		...
		error="在接收数据时发送了错误!\r\n";
		m_pDlg->DisplayInfo(S_CLOSE);   //显示信息
	}
}
...
void CPop3ClientSocket::Close()
{
	...
	state=FIRST;
	CAsyncSocket::Close();
	error="执行CPop3ClientSocket::Close()命令，Not connected to server\r\n";
}
...
m_pDlg->m_mailInfo+=s;
s.Format(_T("信件号%d: ,信件大小%d, 信件内容： %s   信件结束%c%c"),
