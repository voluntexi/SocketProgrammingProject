...
int CMainFrame::BuildFavoritesMenu(LPCTSTR pszPath, int nStartPos, CMenu* pMenu)
{
	...
	// now scan the directory, first for .URL files and then for subdirectories that may also contain .URL files
...
void CMainFrame::SetAddress(LPCTSTR lpszUrl)
{
	...
	// so make sure the text in the address bar is up to date and stop the animation.
...
{
	// Start the animation.  This is called when the browser begins to navigate to a new location
...
{
	// gets called when an item in the Address combo box is selected just navigate to the newly selected location.
	CString str;
...
{
	// gets called when an item is entered manually into the edit box portion of the Address combo box.
	// navigate to the newly selected location and also add this address to the list of addresses in the combo box.
	CString str;
